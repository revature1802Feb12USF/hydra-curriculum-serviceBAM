...
/**
 * Rest Assured tests for the CurriculumController endpoints.
 * 
...
public class CurriculumTests {
    ...
    public void testGetAllCurriculums() {
        RestAssured.get("http://localhost:9001/api/v2/curricula/all").then()
                        .assertThat().body("[0].id", equalTo(100));

        RestAssured.get("http://localhost:9001/api/v2/curricula/all").then()
                        .assertThat().body("[1].id", equalTo(101));

        RestAssured.get("http://localhost:9001/api/v2/curricula/all").then()
                        .assertThat().body("[2].id", equalTo(102));

        RestAssured.get("http://localhost:9001/api/v2/curricula/all").then()
                        .assertThat().body("[3].id", equalTo(103));
    }
    ...
    public void testGetCurriculums() {
        RestAssured.get("http://localhost:9001/api/v2/curricula/?ids=101&ids=102")
                        .then().assertThat().body("[0].id", equalTo(101));

        RestAssured.get("http://localhost:9001/api/v2/curricula/?ids=101&ids=102")
                        .then().assertThat().body("[1].id", equalTo(102));

        RestAssured.get("http://localhost:9001/api/v2/curricula/?ids=101&ids=102")
                        .then().assertThat().body("[2].id", equalTo(null));
    }
    ...
    public void testGetAllCurriculumSubtopicIds() {
        ...
        RestAssured.get("http://localhost:9001/api/v2/curricula/100/subtopics")
                        .then().body("", hasItems(1000, 1001, 1002, 1003));
    ...
    public void testMarkCurriculumAsMaster() {
        ...
        RestAssured.get("http://localhost:9001/api/v2/curricula/?ids=119")
                        .then().body("[0].masterVersion", equalTo(true));
    ...
    public void testAddCurriculum() {
        ...
        Curriculum curriculum = new Curriculum(null, "CustomCurriculum", 1, 1,
                        1, new Date(), 10, true);

        JsonNode jsonNode = JsonNodeFactory.instance.pojoNode(curriculum);

        RestAssured.given().contentType("application/json").body(jsonNode)
                        .when().post("http://localhost:9001/api/v2/curricula/")
                        .then().statusCode(200);
    ...
    public void testDeleteSubtopics() {
        RestAssured.delete(
                        "http://localhost:9001/api/v2/curricula/100/subtopics/?ids=1000")
                        .then().statusCode(200);
    }
    ...
    public void testDeleteCurriculums() {
        RestAssured.delete(
                        "http://localhost:9001/api/v2/curricula/?ids=120&ids=121")
                        .then().statusCode(200);
    }
     ...
     */
    @Test
    public void testUpdateCurriculum() throws JsonParseException,
                    JsonMappingException, IOException {

        ObjectMapper mapper = new ObjectMapper();

        String curriculumJsonStr = RestAssured
                        .get("http://localhost:9001/api/v2/curricula/?ids=101")
                        .body().asString();
        Curriculum[] curriculums = mapper.readValue(curriculumJsonStr,
                        Curriculum[].class);
        ...
        RestAssured.given().contentType("application/json").body(jsonNode)
                        .when().patch("http://localhost:9001/api/v2/curricula/")
                        .then().statusCode(200);

    }

    /**
     * Rest Assured Test to replace a curriculum (another way to update a
     * curriculum).
     * 
     * 
     * @author Seth Maize (1802-Matt)
     */
    @Test
    public void testReplaceCurriculum() throws JsonParseException,
                    JsonMappingException, IOException {
        ObjectMapper mapper = new ObjectMapper();

        String curriculumJsonStr = RestAssured
                        .get("http://localhost:9001/api/v2/curricula/?ids=111")
                        .body().asString();
        Curriculum[] curriculums = mapper.readValue(curriculumJsonStr,
                        Curriculum[].class);
