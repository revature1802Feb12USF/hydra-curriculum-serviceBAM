...
public final class ReflectionUtils {
    ...
    /**
     * Generates a list of possible getter method names for the field
     * {@code f}.<br>
     * <br>
     ...
     * 
     * @param f
     *            The field to generate default getter method names for.
    ...
    private static List<String> generateDefaultGetterNames(Field f) {
        ...
        final String[] PREFIXES = f.getType().equals(Boolean.class)
    ...
    /**
     * Generates a list of possible setter method names for the field
     * {@code f}.<br>
     * <br>
     * Default names follow the convention: <br>
     * <ul>
     ...
     * 
     * @param f
     *            The field to generate default setter method names for.
     ...
     * 
     * @param field
     *            The field to find a setter method for.
     * @return The setter method if one was found; otherwise, {@literal null} is
     *         returned.
     */
    public static Method findSetter(Field field) {
        ...
        String setterName = setterAnnotation == null ? ""
                        : setterAnnotation.value();
        Method setterMethod;
        ...
        if (setterName.isEmpty()) {
            ...
            Optional<String> defaultSetterName = defaultSetterNames.stream()
                            .filter((String name) -> {
                                try {
                                    return Objects.nonNull(declaringClass
            ...
            if (!defaultSetterName.isPresent())
                ...
                try {
                    setterMethod = declaringClass.getDeclaredMethod(
                                    defaultSetterName.get(), field.getType());
            ...
            try {
                setterMethod = declaringClass.getDeclaredMethod(setterName,
                                field.getType());
            } catch (NoSuchMethodException | SecurityException ex) {
                setterMethod = null;
            }
     ...
     * 
     * @param field
     *            The field to find a getter method for.
     * @return The getter me thod if one was found; otherwise, {@literal null}
     *         is returned.
     */
    public static Method findGetter(Field field) {
        ...
        String getterName = getterAnnotation == null ? ""
        ...
        if (getterName.isEmpty()) {
            ...
            Optional<String> defaultGetterName = defaultGetterNames.stream()
                            .filter((String name) -> {
                                try {
                                    return Objects.nonNull(declaringClass
                                                    .getDeclaredMethod(name));
            ...
            if (!defaultGetterName.isPresent())
                ...
                try {
                    getterMethod = declaringClass
                                    .getDeclaredMethod(defaultGetterName.get());
                } catch (NoSuchMethodException | SecurityException ex) {
                    getterMethod = null;
                }
            }
     ...
     * 
     * @param clazz
     *            The class to produce an inheritance list from.
    ...
    /**
     * Copies all non-null fields from {@code src} to {@code target}. Fields
     * which do not have a valid getter and/or setter are ignored. Fields
     * annotated with {@link IgnoreField} are also ignored during the copy.
     * 
     * 
     * @author Ricky Baker (1802-Matt)
     * 
     * @param <T>
     *            The type of {@code src} and {@code target}.
     * 
     * @param target
     *            The target object to copy fields to.
     * @param src
     *            The source object to copy non-null fields from.
     * 
     * @return {@code src} if {@code target} is null; otherwise, {@code target}
     *         is returned.
     * 
     * @throws IllegalArgumentException
     *             {@code target} and {@code src} are not the same type.
     * 
     */
    public static <T> T deepCopyNonNull(T target, T src)
                    throws IllegalArgumentException {
        ...
        if (!target.getClass().equals(src.getClass())) {
            ...
            throw new IllegalArgumentException(msg.format(new String[] { "to",
                            target.getClass().getName(), "from",
        ...
        clazzes.forEach(clazz -> {
            ...
            fields.stream().filter(field -> { // Filter out ignored fields.
                IgnoreField ignoreAnnotation = field
                                .getAnnotation(IgnoreField.class);

                if (ignoreAnnotation == null) {
                    return true;
                } else {
                    List<ReflectionOp> ops = Arrays
                                    .asList(ignoreAnnotation.value());
                    return ops.stream().noneMatch(op -> { // true if no ignore
                                                          // flag found.
                        switch (op) {
                        case ALL:
                        case COPY:
     ...
     * 
     * @param <T>
     *            The type of {@code a} and {@code b}.
     * @param a
     *            The object to compare to {@code b}.
     * @param b
     *            The object to compare to {@code a}.
     * 
     * @return {@literal true} if all fields of {@code a} and {@code b} are
     *         equal. Otherwise, {@literal false}.
     */
    public static <T> boolean testEquality(final T a, final T b) {
        ...
        if (a == b) { // Both null or refer to the same item.
            isEqual = true;
        } else if (a == null || b == null) { // Either are null (cannot be
                                             // both).
            isEqual = false;
        } else if (!a.getClass().equals(b.getClass())) { // Not of the same
                                                         // lowest-level class.
            isEqual = false;
        } else { // Test each field for equality via the field's equals()
                 // method.
            List<Field> fieldList = Arrays.asList(a.getClass().getFields());

            isEqual = fieldList.stream().filter(field -> { // Filter out ignored
                                                           // fields.
                IgnoreField ignoreAnnotation = field
                                .getAnnotation(IgnoreField.class);
